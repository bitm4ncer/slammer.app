...
import { hydrate, noise2 } from '../../vector/_helpers.js';
...
export default {
  ...
  icon: 'braille',
  category: 'pattern',
  description: 'Fill with dot stippling',
  pro: true,
  pack: 'dots-pack',
  ...
  processPaths(paths, params, ctx) {
    ...
    for (const rec of paths) {
      ...
      const bx = b.x, by = b.y;
      const jAmp = jitter * sp * 0.5;     // hoisted out of the inner loop
      // Reuse a single point for the contains() probe — saves one
      // paper.Point allocation per grid cell.
      const probe = new paper.Point(0, 0);
      for (let j = 0; j < rows; j++) {
        const yy = by + j * rowH;
        const xOff = hex && (j & 1) ? sp / 2 : 0;
        for (let i = 0; i < cols; i++) {
          const xx = bx + i * sp + xOff;
          // Jitter inside the cell (deterministic via noise2).
          const jx = noise2(xx * 0.07, yy * 0.07, seed) * jAmp;
          const jy = noise2(xx * 0.11, yy * 0.05, seed + 7) * jAmp;
          const px = xx + jx;
          const py = yy + jy;
          probe.x = px; probe.y = py;
          if (cp.contains(probe)) {
