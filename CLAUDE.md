...
- **After each change, bigger step, or finished feature, give a short summary** that covers where / what / why — but written naturally, NOT with "Where:" "What:" "Why:" labels.  Two or three sentences max, plain prose. The user wants to follow along without re-reading the diff.
  - Skip the summary on small back-and-forth (clarifications, one-line acknowledgements, single-tool ops the user can clearly see).
  - Use it after: a commit, a subagent batch finishing, a parked-bug decision, a server restart, a milestone closing.
  - The summary should mention which file(s) were touched, what the user-visible effect is, and why it matters — woven into one short paragraph, not three labelled lines.
