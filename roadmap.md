...
- [x] **Canvas drag-and-drop full-res loading indicator** ✅ shipped (`55d6fbc`) — renderer.js emits `layer:firstPainted` on the first paintLayerSync commit where `dstCanvas > 1×1`; layer-panel.js swaps the placeholder thumb for an inline ring spinner while the layer is unpainted; new `src/ui/drop-loader.js` shows a small ring at the drop's world coords (FIFO-binds to next `layer:added`), fades in after 200 ms so fast drops don't flash, fades out on `layer:firstPainted`. Cache-hit hydration pre-marks layers as painted so reload doesn't flash. Undo/redo doesn't re-flash because `firstPaintedLayerIds` retains the id and dstCanvas stays >1×1.
...
- [x] **Named color variables** — Variables tab in the colour hub (`src/ui/color-hub.js`). User types a name, hits +, captures the current picker colour under `--name` (auto-prefixes the dashes). Each row: drag-source swatch (uses `application/x-slammer-color` + new `application/x-slammer-color-var` MIME), inline rename, inline hex edit (`#RRGGBB`, Enter / blur commits, Esc reverts). Right-click removes. Click a row's swatch enters **variable-edit mode** — banner across the popover top ("Editing --brand · [×]"), the picker (hue ring + HSL triangle + hex + RGB + alpha) writes to the variable instead of the active slot. Esc or × exits. Storage layer was already in place from 23a (`getVariables / setVariable / removeVariable / renameVariable / onVariablesChange` in `colors.js`). **Live propagation**: when a variable's value changes, `selection-style.js#propagateVariablesToLayers` walks `doc.layers` and rewrites every `fill.color` / `stroke.color` / `text.color` carrying a matching `colorVar` binding. **Visibility**: bound layers surface a `--name` badge under their slot chip in the colour hub when selected (via `getEffectiveStyle().fillColorVar` / `strokeColorVar` + `[data-var]::after` CSS rule). Open: assignability to gradient stops + Color Overlay tint (the binding model supports it, the slot-chip badge doesn't yet show gradient-stop bindings).
...
- [x] **Gradient Map** — shipped as `src/plugins/filters/gradient-map/index.js` (~89 LOC). Stops-based 256-entry LUT, per-pixel Rec 601 luminance, `gradientStopsRow` UI helper accepts drops from the Gradient Library plugin. Category `color`, type `filter`. No per-effect Mix / Blend Mode knob — slot-level dry/wet handles it per plugin-contract rule.
- [ ] **Gradient Map — Preserve Luminance toggle** (low-priority polish, nice-to-have) — adds a `preserveLuminance` boolean to `defaultParams()`. When enabled: convert each pixel RGB → HSL, replace H + S with the gradient sample's HSL but keep the source pixel's L. Photoshop-classic control that keeps tonal modulation (e.g. skin shading) intact under aggressive recolouring. ~15 LOC addition, fast path stays untouched when off. Not on the critical path — ship if/when there's a Photo workflow request.
...
## PHASE 31 — Project Tabs / Multi-Project Workspace 🆕

> **Intent**: let the user open multiple projects at once and switch between them via an in-app tab bar — the way Photoshop, Affinity, VS Code do it. Today the editor is single-project: open project A, switch to project B, A is gone from the session (autosaved but not held in memory). This phase keeps multiple projects warm in the workspace simultaneously.

### Why now / why not browser tabs

Browser tabs ARE technically a working answer (each tab = full slammer.app instance, cross-tab clipboard via the `BroadcastChannel('slammer:clipboard')` we already planned in Phase 25). But they have downsides: each tab pays the full boot cost (~500 ms + asset decode), nothing visually ties them together, and the user has to navigate browser chrome to switch. In-app tabs solve all three: shared boot, visible tab strip, instant switching, no browser chrome interaction.

### v1 architecture — logical tabs over one Konva stage

Don't multiply Konva stages — one stage stays the canvas, the doc state behind it swaps on tab change. **Single renderer + single Konva stage + per-tab serialised doc state in memory.** Pragmatic, low-RAM, low-refactor.

- **`docManager`** (new module, replaces the singleton `doc` export): keeps `openTabs: Map<projectId, TabState>` and `activeProjectId: string`. Each `TabState` holds `{ docState, undoStack, redoStack, viewTransform, activeLayerId }`.
- **`window.__slammer.doc`** becomes a getter that returns the active tab's doc instance (proxy or live re-bind). Plugins that reference `__slammer.doc` keep working — they just see whichever doc is active.
- **Tab switch flow**: (1) serialise current doc → `openTabs.get(activeId).docState = doc.state.clone()`; (2) flush autosave for current; (3) load `openTabs.get(targetId).docState` into the `doc` instance via the existing `doc.load(state)` path; (4) renderer's `doc:loaded` handler diff-patches Konva nodes (already implemented in `patchLayerNodes` from `a6ae171`).
- **Per-tab undo history** — each `TabState` carries its own `past[] / future[]` stacks. On tab switch, swap the history's stacks. No history bleed between tabs.
- **View transform per tab** — zoom + pan persist per tab so switching back to a project lands the canvas where the user left it.

### UX

- **Tab bar location**: top of the canvas area, above the rulers (below the global toolbar). Horizontal strip, scrollable when many tabs.
- **Tab visual**: small project thumbnail + name. Active tab has the accent border / highlighted background. Hover shows close `×`.
- **Tab actions**:
  - Single click → switch to tab
  - Middle-click OR `×` button → close tab (confirm if unsaved-since-last-autosave; autosave-equal projects close silently)
  - Right-click → context menu: Close · Close others · Close right · Duplicate tab · Send layer here · Rename project · Save as new file
  - Drag-to-reorder
  - Double-click empty space at end of strip → "New blank project" tab
- **Project Manager popover** (existing) gains a **"Open in new tab"** action button next to the existing "Open" action. The default "Open" still replaces the current tab (= existing behaviour, no surprise for muscle memory); "Open in new tab" adds a sibling.
- **Keyboard shortcuts** (route through Phase 21b Shortcut Manager when it ships):
  - `Ctrl+T` — new blank project in a new tab
  - `Ctrl+W` — close active tab
  - `Ctrl+Shift+T` — reopen last closed tab (within session)
  - `Ctrl+Tab` / `Ctrl+Shift+Tab` — next / previous tab
  - `Ctrl+1` … `Ctrl+9` — jump to tab N
- **Empty state**: if all tabs are closed, show the "Drop an image or open a project" landing screen (existing welcome state).

### Persistence

- `slammer:openTabs` (localStorage) — array of project ids currently open. Restored on app boot.
- `slammer:activeTabIndex` — restored active tab.
- Autosave keeps firing for the ACTIVE tab on every change (existing). Background tabs autosave when last edited, not on every paint — they're already in stable state.
- The serialised `TabState` for inactive tabs lives in memory only — it's NOT persisted as a giant blob. Each tab's project doc still lives in IndexedDB independently; the tab list is just "which projects to reopen on next boot". Closing the browser closes all tabs cleanly; next boot reopens them in the same order.

### Interaction with existing features

- **Plugins** that read `window.__slammer.doc` continue to work — getter pattern ensures they always see the active doc. **Audit needed**: any plugin that captures `doc` in a closure (not on every call) will get a stale ref after tab switch. Verify fal.ai, Unsplash, Pexels, Met, Color Hub.
- **fal.ai in-flight generations** (`activeGenerations` registry, `e68f160`) currently key jobs by `pluginId` + `modelId`. With multiple tabs, a job must also carry `projectId` so the result lands in the originating tab's doc, not whichever tab is active when the generation completes. Update `activeGenerations.start({ ... , projectId })` and the import path uses `docManager.getDoc(projectId).importImage(...)`.
- **Cross-project clipboard** (Phase 25 entry, Ctrl+C/V with `BroadcastChannel`) becomes the cross-tab clipboard for free — same mechanism. v2 (drag layer across tabs) is the UI sugar on top.
- **Color Hub state** (`window.__slammer.colors`) — shared across tabs by default (matches Affinity / Photoshop). The active fill / stroke persists when switching tabs. Per-tab colour state is overkill in v1.
- **Plugin windows** (Unsplash, fal.ai etc.) — shared across tabs. A single open Unsplash window applies the user's drag-out to whichever tab is active. Don't multiply windows per tab.
- **Settings** — shared (theme, accent, API keys, snap/grid prefs). Not per-tab.
- **Active tool** — per-tab. Switching from a project where you were using the Pen back to a project where you were on Select restores that.

### Sub-deliverables (v1)

- [ ] **`docManager` refactor** — extract the singleton `doc` export from `src/core/document.js` into a `docManager` with `openTab(id)`, `closeTab(id)`, `switchTab(id)`, `activeDoc` getter, `openTabs` getter. Existing `window.__slammer.doc` becomes a live proxy. (~300 LOC, touches main.js + every place that imports `doc`.)
- [ ] **Per-tab `TabState`** — `{ docState, undoStack, redoStack, viewTransform, activeLayerId, activeTool }`. Serialise on tab leave, hydrate on tab enter.
- [ ] **Per-tab undo history** — `src/core/history.js` becomes a factory `createHistory()` per tab; `docManager` swaps the active history on tab change. (Snapshot-based today; the History v2 command-pattern overhaul tracked in Phase 19 Cluster F doesn't change this design.)
- [ ] **Tab bar UI** — new `src/ui/tab-bar.js`. Horizontal strip above rulers. Thumbs (from layer-panel's existing thumb pipeline) + names + close-X + drag-to-reorder. Sortable.js already in the project from Phase 5; reuse.
- [ ] **Project Manager "Open in new tab"** — add the button to the existing actions strip in `src/ui/project-menu.js`. Wires to `docManager.openTab(id, { activate: true })`.
- [ ] **Keyboard shortcuts** — register `Ctrl+T / Ctrl+W / Ctrl+Tab / Ctrl+1-9 / Ctrl+Shift+T` in the Shortcut Manager (Phase 21b prerequisite). If Phase 21b hasn't landed yet, wire directly to toolbar.js for now and migrate later.
- [ ] **Persistence** — `slammer:openTabs` + `slammer:activeTabIndex` on every tab change, restored on boot.
- [ ] **Plugin closure audit** — scan every plugin that touches `window.__slammer.doc`; fix any that captures the reference once instead of reading per-call. Patch fal.ai's `activeGenerations` to carry `projectId`.
- [ ] **Empty state** — when all tabs are closed, show the existing landing screen.

### Sub-deliverables (v2 / later)

- [ ] **Drag-layer-across-tabs** — drag a layer card from the Layer Stack onto a tab in the tab bar → layer is copied into that project (clipboard-style serialise + paste, already designed in Phase 25 "Cross-project layer/group transfer via clipboard").
- [ ] **Detach tab to new browser window** — `window.open(slammer.app/?project=<id>)`. Advanced; defer unless requested.
- [ ] **Tab groups** — colour-coded tab clusters (Chrome-style). Probably overkill; defer.
- [ ] **Unsaved-changes indicator** — small dot next to tab name when dirty (since last autosave commit).

### Open questions

- **Tab count limit?** No hard cap in v1. Soft warning at >12 ("This many open tabs may slow the app — close some?"). Each background tab's `TabState` is small in memory (project doc state is already in IndexedDB; we just hold a clone), but the undo stacks per tab can add up — monitor.
- **What about Phase 18 "Artboards / Pages"?** Different concept: pages live INSIDE a project, tabs hold separate projects. They don't conflict but the UI placement (tab strip vs page tabs) needs distinguishing. Tab strip = projects (across IndexedDB), page tabs (deferred) = pages within the active project. Place project tab strip above page tab strip if both ever ship.
- **What about Phase 24 multi-frame export?** Frames live inside a project too. Same answer as artboards — tabs are project-scoped, frames are within-project.

### Effort estimate

~5-7 days for v1 (the `docManager` refactor + per-tab state + tab bar UI + plugin audit). Per-tab history adds ~1 day. Keyboard shortcuts depend on Phase 21b landing first or get wired ad-hoc. Drag-across-tabs is a separate v2 sprint.

### Prerequisite

Nothing strict. Could ship after Phase 19 bug-bash polish settles. Phase 21b (Shortcut Manager) is nice-to-have so the new keyboard shortcuts register cleanly, but not required. **Strong candidate for the "first big workflow upgrade after v1 launch"** — moving from single-project to multi-project workspace is a noticeable maturity step.
...
| Pack | Items | Status |
|---|---|---|
| **Glitch Pack** | Datamosh · JPEG Compression · **Time Smear** (new) · **Glyph Destruction** (new) | Datamosh + JPEG migrated; Time Smear TBD (combined Echo + Trails + Slit-Scan); Glyph Destruction TBD (text-outline destruction) |
| **Raster Pack** | Dither · Halftone (raster) · **ASCII** (new) · **Riso Separation** (new) | Dither migrated; raster Halftone + ASCII TBD; Riso Separation TBD (grainy multi-color separation) |
...
| **Generative Pack** (new) | **Reaktor** · **Slime Mold** · **Kaleidoscope** | New pack — "ornamental algorithms" theme; all three TBD (Reaktor = reaction-diffusion port from REAKTOR2 repo, Slime Mold = physarum agent sim, Kaleidoscope = recursive n-fold mirror) |
...
- [ ] **PHOSIMP-inspired effects + meta-features audit** (research-derived) — the Steam app PHOSIMP (KanjiCoder, $1.29, June 2025) ships 1111+ numbered filters with a cult following around its trippy / kaleidoscopic / tiling-pattern output. Six candidates that close gaps in our catalog without overlapping anything already shipped:
  1. **Auto-Tile wrap mode** 🟢 META — meta-feature, not a single effect. Adds a "tiled output" toggle (global or per-effect) that runs the existing effect pipeline in toroidal-wrap sample mode so output tiles seamlessly. Implementation: pre-pass pads the source with wrapped copies on each side, runs the existing effect, crops back. ~80 LOC, no per-effect rewrite needed. Watch the rasteriser pad logic in CLAUDE.md — add a parallel "tile pad = 0, sample wraps instead" branch. **Highest leverage of the batch**: multiplies every effect we already ship into a wallpaper / texture generator.
  2. **Kaleidoscope / Radial Symmetry** 🟢 FREE (Stylize) — N-segment wedge mirror with adjustable centre, rotation, segment count. Pure inverse-mapping pixel loop. Pairs with Auto-Tile for "infinite wallpaper" output. ~150 LOC.
  3. **Plasma / Trigonometric-noise generator** 🟢 FREE (Render category — new) — multi-octave `sin(ax + by + c·luma)` field. With Gradient Map for palette = classic plasma / demoscene look. Canvas 2D suffices for ≤1024², WebGL fragment shader for live-preview at 4k+. ~120 LOC Canvas / ~80 GLSL.
  4. **Per-Channel Filter Chain** 🟢 META — architectural feature: every layer-effect slot gains a `channelMask` field (R / G / B / A / RGB / RGBA, default RGBA). Renderer splits ImageData into channel planes, runs the slot's effect on the masked planes only, recomposites. Each plugin's `process()` stays unchanged — the renderer just feeds it a masked view. **Massive creative multiplier** on every existing effect (e.g. "Blur only the red channel", "Pixelsort the blue plane while leaving red+green alone"). UI: small chip on every effect card showing the active channel mask, click to cycle / open picker. ~400 LOC including UI + history wiring (channelMask must round-trip through `doc.setEffectParams`-equivalent so undo + autosave capture it).
  5. **Reversible Glitch (XOR + bit-plane shuffle)** 🟦 ADD-ON (Glitch Pack candidate) — bijective glitch effect: seeded XOR noise on RGB byte planes, optional bit-plane shuffle. Applying twice with the same seed = identity (the f / r reversibility constraint PHOSIMP commits to). Niche aesthetic (harsh datamosh-without-video look), fits the Glitch Pack identity. ~100 LOC. Seed + algorithm-id serialise into effect params so the inverse is just the same effect with `invert: true`.
  6. **Domain-Warp (curl / simplex-noise driven)** 🟢 FREE or 🟦 ADD-ON (Liquid Pack candidate) — noise-driven displacement field. You have Twirl/Ripple/Bulge (all analytic radial) but no noise-driven warp. Bilinear-sample source at displaced coords; vector field from 2D simplex noise + optional curl. Distinct from Liquid Gradients (which renders noise → colour) and Liquify (procedural displacement also planned). Pairs beautifully with Posterize + Gradient Map for psychedelic banding. ~200 LOC Canvas / ~120 GLSL. May overlap with the Monolab "Vector Field Warp" candidate — pick one approach when implementing.
  
  **Strategic note**: candidates #1 (Auto-Tile) and #4 (Per-Channel Chain) are NOT effects — they're meta-features that upgrade every existing effect in the catalog. Build them BEFORE the individual effects below to maximise leverage. Together they enable outputs that aren't reachable by any single new effect.
...
- [ ] **Reaktor** 🟪 ADD-ON (Generative Pack) — Stylize/Render. Gray-Scott reaction-diffusion live on the active layer. Port of the existing REAKTOR2 codebase (TypeScript / Vite / WebGL2 with ping-pong framebuffers, über-shader `grayscott.frag.glsl`, repo: github.com/bitm4ncer/REAKTOR2). **Twist vs the standalone tool**: active slammer layer acts as a **feed-map** — photo / text / vector → the simulation grows along the luminance / edge structure of the input (text becomes coral, photos become organic patterns). **Controls**: Feed-rate (0–0.1), Kill-rate (0–0.08), Diffusion-A, Diffusion-B, Steps-per-frame (1–60), Simulation-resolution (64–1024), Presets (Coral / Spots / Mitosis / Spirals / Worms / Stripes — ported directly from REAKTOR2). **Source knobs**: Feed-map mode (Off / Luma / Edges / Alpha) with strength slider. **Render modes** (from REAKTOR2): Color-Gradient (via Gradient Library stops when available, otherwise default ramp), Halftone, Threshold, ASCII glyph-atlas. No Mix knob (slot-level handles dry/wet). **Output**: after stabilisation, snapshot back as bitmap, layer type preserved. **Architecture**: WebGL2 ping-pong reuse from REAKTOR2's `src/simulation/`; `src/rendering/` pipeline-selector adapted to slammer's existing `OffscreenCanvas` output convention. Shared feed-map sampler with Slime Mold via `_shared/feed-map-sampler.js`. **Effort**: M (3–4 days — port the GLSL, wrap in plugin manifest, feed-map integration, persist state for animation pause/resume).
- [ ] **Slime Mold** 🟪 ADD-ON (Generative Pack) — Render/Distort. Agent-based Physarum simulation (Jeff Jones 2010 / Sage Jenson style) on WebGL2 transform-feedback. Thousands of particles follow a deposit / sense / decay loop, leaving trails. **Active layer** acts as a **lure map** (text / logo → slime wanders along the form, photo → slime follows luminance gradients). **Controls**: Sense-angle (0–180°), Sense-distance (1–50), Turn-angle (0–90°), Step-size (0.1–10), Deposit-strength (0–1), Decay-rate (0–0.1), Diffusion-sigma, Agent-count (1k–500k, GPU-bound), Iterations (1–10 per frame). **Lure modes**: Off (free-roam) / Luma-attract / Edge-follow / Alpha-attract. **Presets**: Veins (low decay, high deposit), Mycelium (high turn), Cosmic-Web (low sense-angle), City-Network (high sense-distance), Hairlines (low step-size). **Render modes**: trail-map raw, Color-mapped (Gradient Library), Threshold-bone, Inverted-galaxy. **Snapshot trigger**: user clicks Capture → current trail map is written into the layer as a bitmap. **Architecture**: WebGL2 transform-feedback for agent update, ping-pong texture for trail decay, dedicated compute pass. Shares `_shared/feed-map-sampler.js` with Reaktor. **Effort**: L (5–7 days — new WebGL2 infrastructure is the biggest investment; agent tuning is polish after).
- [ ] **Glyph Destruction** 🟦 ADD-ON (Glitch Pack) — Stylize, text-only. Operates on glyph outlines **before rasterisation** (reuses `src/ui/vector-tools/text-to-path.js` path-extraction). Sub-modes: (a) **Melt** — per-glyph gravity-drip, vertical drooling with random velocity per segment; (b) **Fracture** — path-split along internal lines + jitter of fragments; (c) **Bitcrush-Outline** — quantise bezier control points to a pixel grid (1–16 px steps), produces an 8-bit look; (d) **Decay** — random erosion of path segments, letter-decay equivalent; (e) **Drip** — downward path growth, each bottom anchor sprouts downward; (f) **Liquify** — domain-warped outline displacement, organic flow. **Shared controls**: Strength, Per-Glyph-Stagger (Random / Sequential / Reverse), Seed, Edge-mode (clamp/wrap), Affect-Letters-Only (off-by-default — punctuation may destroy too). **Manifest gates**: `supportedLayerTypes: ['text']`. **Architecture**: uses Paper.js (already installed for Phase 13c booleans) for path editing; output is a new glyph-outline list the text rasteriser then draws. Does NOT touch the OT engine — opentype.js extraction stays unchanged, we only edit the resulting path set. **Distinction**: existing **Glitch Text Builder** (Type Pack, F5 line ~483) operates at raster / sub-pixel level (Char-Replace, Scanline, Chromatic Shift). Glyph Destruction operates on outlines (vector before raster). No overlap; both can co-exist on the same text layer. **Effort**: M-L (4–5 days for all 6 modes polished; 2–3 days for MVP with Melt + Bitcrush + Decay).
- [ ] **Riso Separation** 🟥 ADD-ON (Raster Pack) — Render, Multi-Output. Image → N spot-color layers with **authentic grainy riso look** (NOT a clean CMYK halftone — the whole point is the imperfect-print aesthetic). Multi-output filter: produces N **real layers** instead of one output, each with its own manifest tag `riso-channel: <color-id>`. **Spot-color picker**: 21 real Riso inks from the official Riso palette (Fluorescent Pink, Federal Blue, Hunter Green, Yellow, Black, Fluorescent Orange, Cornflower, Sea Blue, Cranberry, etc.) plus custom RGB. **Separation engine**: photo is decomposed in HSL, then per-color slot matched via CIELAB Delta-E onto the chosen inks. Optional dither-in-separation pass (riso-typical). **Grainy-look requirements** (this is the point):
  - **Per-layer halftone** with characteristic angles (classic: Magenta 15°, Cyan 75°, Yellow 90°, Black 45°) — user can override per layer for authentic riso moiré.
  - **Ink-stipple texture** — sub-pixel noise overlay per layer, simulating ink distribution on the riso master sheet. Not a simple grain overlay but: per-pixel deposit probability, filtered through a riso-typical noise spectrum (more low-frequency variation than white noise).
  - **Misregistration** — random offset vector per layer, default ±2–4 px, configurable up to ±20 px. Each layer has its own seed → reproducible, plus a "Reroll all layers" button for quick A/B.
  - **Paper-texture underlay** — built-in mid-grey-warm paper-tone layer, optional, with subtle fibre noise. User can upload custom paper textures (riso studios use ~8 standard stocks).
  - **Ink-opacity curve** — per layer, a 2-point curve controlling ink saturation in highlights vs shadows. Riso inks are never 100 % opaque — the semi-translucent stack IS the look.
  - **Knockout vs Overprint** — per-layer toggle. Knockout = layer writes 0 into lower layers (clean separation); Overprint = layer multiplies onto what's beneath (real riso stacking).
- **Optional "Color Squash" button**: forces color pull from a narrower ink palette (e.g. 2-color riso) — reduces to 2/3/4/5/6 selectable inks.
- **Output**: produces an N-layer stack with correct z-order (black on top by default). Each layer keeps a plugin reference, so the user can re-tune the separation algorithm per layer (that's the workflow anchor).
- **Architecture**: first plugin with a multi-layer-output pattern. Plugin host needs a new `outputType: 'multi-layer'` manifest convention. We define the pattern here; other plugins (CMYK separation, frequency separation) will reuse it later.
- **Reference implementations**: [risottostudio.com](https://www.risottostudio.com/) (visual reference), [stencil.wiki/colors](https://www.stencil.wiki/colors) (ink values), open-source riso-prep tools for separation logic.
- **Effort**: L–XL (6–8 days — multi-output architecture + 5 sub-engines: color-match, halftone, stipple-noise, misregister, ink-curve. Realistically two sprints).
- [ ] **Kaleidoscope** 🟪 ADD-ON (Generative Pack) — Stylize/Distort. Classic n-fold mirror PLUS **recursive depth** — at depth ≥ 3 it turns into a mandala generator rather than a kaleidoscope. Third member of the Generative Pack alongside Reaktor + Slime Mold — all three produce ornamental patterns via algorithm (Reaktor = chemistry sim, Slime Mold = agent sim, Kaleidoscope = symmetry transform). Synth mode (see below) makes Kaleidoscope work standalone without an input layer, so it's genuinely "generative" too. **Controls**: Folds (3–24, default 6), Center-X / Center-Y (canvas-relative puck), Rotation (0–360°), Wedge-source-width (fraction of the input sector), Recursive-depth (0–6 — at 0 it's a classic kaleidoscope, at 6 deeply fractal), Recursive-scale (per-iteration scaling, 0.4–1.0), Recursive-rotation (per-iteration drift), Edge-mode (mirror/wrap/clamp/transparent), Color-cycle (optional per-recursion HSL rotate — turns a photo into a mandala rainbow). **Source modes**: Input-layer (default), or **Synth** — with no layer content, internally generates a Liquid-Gradients-style noise pattern and kaleidoscopes THAT (reuse the Liquid Gradients noise engine via `_shared/simplex-multi-octave.js` once Liquid Gradients is extracted). **Preset list**: Classic / Mandala / Snowflake (12-fold) / Y2K Spiral (24-fold + spin) / Underwater (4-fold low-depth) / Galaxy (recursive depth 4 + color-cycle). **Architecture**: single WebGL shader pass (radial warp + UV mirror), depth via iterative texture sampling inside the same shader. Very compact. **Distinction**: no overlap with Twirl/Ripple/Bulge (Liquid Pack — radial filters), no overlap with Mesh Gradient (Gradient Pack — vertex-based color smoothing). Kaleidoscope is coord-transform with symmetry. **Effort**: S–M (2–3 days for fully polished including all presets; 1 day for MVP).
